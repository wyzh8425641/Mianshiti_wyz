    ...
    private void backtrack1(List<List<Integer>> list, List<Integer> tempList, int [] nums, int remain, int start){
        if(remain < 0) return;
            ...
            for(int i = start; i < nums.length; i++){
                tempList.add(nums[i]);
                backtrack1(list, tempList, nums, remain - nums[i], i); // not i + 1 because we can reuse same elements
...
public class combinationSum2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            List<List<Integer>> ret = new Solution1().combinationSum2(candidates, target);
